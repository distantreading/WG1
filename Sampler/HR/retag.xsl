<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="h:p[@class]">
        <xsl:choose>
            ...
            <xsl:when test="@class='Normalcenter'">
                <xsl:if test="matches(.,'[IVX\.]+')"><head><xsl:apply-templates/></head></xsl:if>
                <xsl:if test="matches(.,'\*')"><milestone/></xsl:if>
                
            </xsl:when>
        </xsl:choose>       
    </xsl:template>
    
    <xsl:template match="h:span[@class = 'MsoFootnoteReference']">
        <ref target="{parent::h:a/@href}">
            <xsl:apply-templates/>
        </ref>
    </xsl:template>
    
<xsl:template match="h:a">
        <xsl:apply-templates select="h:span"/>
    </xsl:template>
